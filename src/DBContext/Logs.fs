...
type Logs (connection: SqliteConnection) =
    ...
    member _.insertLog text userId =
        ...
        comm.CommandText <- "insert into Logs (data, userId, timestamp) values ($data, $userid, $timestammp); select last_insert_rowid()"
        comm.Parameters.AddWithValue ("userid",match userId with Some x -> x | None -> DBNull.Value) |> ignore
