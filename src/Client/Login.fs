...
let LoginFormWithErrors values errors =
    let displayError field =
        ...
        | _ ->
            let elems =
                summary [] [Text "Errors"] :: errors
            Some (details [_open] elems)
        
    let createInput inputtype field lbl =
        seq {
            yield label [ _for field] [ Text lbl ]
            yield input [ _type inputtype; _id field; _name field; _required; _value (match List.tryFind (fun x -> fst x = field) values with | Some e -> snd e | None -> "") ]
